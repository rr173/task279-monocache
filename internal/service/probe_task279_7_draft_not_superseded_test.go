...
func TestDraftSnapshotNotSuperseded(t *testing.T) {
	svc := newTestService(t)
	pair := mustConflictPair(t, svc, "draft-keep")
	if _, _, err := svc.CompareCache(pair.reqA.ID); err != nil {
		t.Fatal(err)
	}
	a, err := svc.CreateSnapshot(pair.batch.ID, "first")
	if err != nil {
		t.Fatal(err)
	}
	b, err := svc.CreateSnapshot(pair.batch.ID, "second")
	if err != nil {
		t.Fatal(err)
	}
	if err := svc.PublishSnapshot(a.ID, "first-pub"); err != nil {
		t.Fatal(err)
	}
	gotA, err := svc.GetSnapshot(a.ID)
	if err != nil {
		t.Fatal(err)
	}
	gotB, err := svc.GetSnapshot(b.ID)
	if err != nil {
		t.Fatal(err)
	}
	if gotA.Status != model.SnapPublished {
		t.Fatalf("published snapshot status = %s", gotA.Status)
	}
	if gotB.Status != model.SnapDraft {
		t.Fatalf("other draft became %s, want draft", gotB.Status)
	}
}
