...
func TestConcurrentPublishOneWinner(t *testing.T) {
	...
	pair := mustConflictPair(t, svc, "pub-race")
	if _, _, err := svc.CompareCache(pair.reqA.ID); err != nil {
		t.Fatal(err)
	}
	snapA, err := svc.CreateSnapshot(pair.batch.ID, "a")
	if err != nil {
		t.Fatal(err)
	}
	snapB, err := svc.CreateSnapshot(pair.batch.ID, "b")
	if err != nil {
		t.Fatal(err)
	}
	const workers = 20
	...
	for i := 0; i < workers; i++ {
		...
		go func(i int) {
			...
			if err := svc.PublishSnapshot(id, "pub"); err != nil {
	...
	if published != 1 {
		t.Fatalf("published snapshots = %d, want 1", published)
	}
}
