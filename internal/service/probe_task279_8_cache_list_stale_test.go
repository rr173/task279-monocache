...
func TestCacheListMatchesPersistedMerge(t *testing.T) {
	svc := newTestService(t)
	pair := mustConflictPair(t, svc, "stale-list")
	...
	if conflictBefore == 0 {
		t.Fatal("setup should have a conflict entry")
	}
	if err := svc.MergeAndResolve([]string{pair.reqA.ID, pair.reqB.ID}); err != nil {
		t.Fatal(err)
	}
	after, err := svc.ListCache()
	if err != nil {
		t.Fatal(err)
	}
	for _, e := range after {
		if e.Status == model.CacheConflict {
			t.Fatalf("list cache still shows conflict after merge: %+v", e)
		}
	}
}
