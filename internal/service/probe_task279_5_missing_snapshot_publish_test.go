...
func TestMissingSnapshotPublishNoPanic(t *testing.T) {
	svc := newTestService(t)
	if err := svc.PublishSnapshot("snap-missing", "note"); err == nil {
		t.Fatal("missing snapshot publish should fail")
	} else if !errors.Is(err, model.ErrNotFound) {
		t.Fatalf("publish missing snapshot: %v", err)
	}
	if _, err := svc.CreateSnapshot("batch-missing", "note"); err == nil {
		t.Fatal("missing batch create snapshot should fail")
	} else if !errors.Is(err, model.ErrNotFound) {
		t.Fatalf("create snapshot missing batch: %v", err)
	}
}
